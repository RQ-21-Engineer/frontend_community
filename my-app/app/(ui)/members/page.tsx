...
export default function MembersPage() {
    ...
    const cardStyles = useMemo(() =>
        Array(members.length).fill(null).map(() =>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {members.map((member, index) => {
                        ...
                        const transformValues = {
                            translateY: currentTransform.translate.includes('translate-y-2') ? 8 :
                                currentTransform.translate.includes('-translate-y') ? -8 : 0,
                            translateX: currentTransform.translate.includes('translate-x-2') ? 8 :
                                currentTransform.translate.includes('-translate-x') ? -8 : 0,
                            rotate: currentTransform.translate.includes('rotate-2') ? 2 :
                                currentTransform.translate.includes('-rotate') ? -2 : 0,
                        };
                            ...
                            >
                                <Link href={`/members/${member.github}`}>
                                    <div className="bg-zinc-900 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer border border-zinc-800">
                                        ...
                                        <div className="p-3">
                                            <h3 className="text-base font-semibold mb-1 truncate text-white">{member.name}</h3>
                                            <p className="text-zinc-400 text-sm mb-2 truncate">{member.role}</p>
